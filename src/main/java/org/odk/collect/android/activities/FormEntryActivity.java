    ...
    private View createView(int event, boolean advancingPage) {
        ...
        switch (event) {
            ...
            case FormEntryController.EVENT_END_OF_FORM:
                ...
                final String[] saveName = {formController.getSubmissionMetadata().instanceName};
                final String saveInstacia= formController.getSubmissionMetadata().instanceId;
                BranchSession objBranchSessio1=new BranchSession();
                if(objBranchSessio1.getE_code().equals("")&& objBranchSessio1.getE_code()!="code") {
                    String saveNameUpdateu = formController.getSubmissionMetadataUpdate(objBranchSessio1.getE_code(),objBranchSessio1.getE_rutaaggregate(),objBranchSessio1.getE_nuevo());
                }else{
                    if(objBranchSessio1.getE_code()!="code"){
                        String saveNameUpdateu = formController.getSubmissionMetadataUpdate(objBranchSessio1.getE_code(), objBranchSessio1.getE_rutaaggregate(), objBranchSessio1.getE_nuevo());
                    }
                }
                ...
                if (formController.getSubmissionMetadata().instanceName == null) {
                    ...
                    if (saveName[0] == null && uriMimeType != null
                    ...
                    saveName[0] = formController.getSubmissionMetadata().instanceName;
                    TextView sa = endView.findViewById(R.id.save_form_as);
                ...
                endView.findViewById(R.id.save_exit_button)
                        .setOnClickListener(new OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                if(objB.getE_ID().equals(""))
                                {
                                    CodigoSession codse= new CodigoSession();
                                    if(codse.getcId().equals("")){

                                    }else {
                                        try {
                                            usdbh = usdbh.open();
                                            ContentValues Objdatos = new ContentValues();
                                            Objdatos.put(EstructuraBD.CabecerasCodigos.idAccount, codse.getC_idAccount());
                                            Objdatos.put(EstructuraBD.CabecerasCodigos.code, codse.getC_code());
                                            Objdatos.put(EstructuraBD.CabecerasCodigos.estado, codse.getC_estado());
                                            Objdatos.put(EstructuraBD.CabecerasCodigos.uri,saveInstacia);
                                            Objdatos.put(EstructuraBD.CabecerasCodigos.imei_id, codse.getC_imei_id());
                                            String where = "_ID='" + codse.getcId() + "'";
                                            usdbh.ActualizarTablaCodigos(Objdatos, where);
                                            usdbh.close();
                                        } catch (Exception e) {
                                    ...
                                    try {
                                        usdbh = usdbh.open();
                                        ContentValues Objdatos = new ContentValues();
                                        Objdatos.put(EstructuraBD.CabecerasEngine.idbranch, objB.getE_idbranch());
                                        Objdatos.put(EstructuraBD.CabecerasEngine.idAccount, objB.getE_idAccount());
                                        Objdatos.put(EstructuraBD.CabecerasEngine.externalCode, objB.getE_externalCode());
                                        Objdatos.put(EstructuraBD.CabecerasEngine.code, objB.getE_code());
                                        Objdatos.put(EstructuraBD.CabecerasEngine.name, objB.getE_name());
                                        Objdatos.put(EstructuraBD.CabecerasEngine.mainStreet, objB.getE_mainStreet());
                                        Objdatos.put(EstructuraBD.CabecerasEngine.neighborhood, objB.getE_neighborhood());
                                        Objdatos.put(EstructuraBD.CabecerasEngine.reference, objB.getE_reference());
                                        Objdatos.put(EstructuraBD.CabecerasEngine.propietario, objB.getE_propietario());
                                        Objdatos.put(EstructuraBD.CabecerasEngine.uriformulario, saveInstacia);
                                        String where = "_ID='" + objB.getE_ID() + "'";
                                        usdbh.ActualizarTabla(Objdatos, where);
                                        usdbh.close();
                                    } catch (Exception e) {
                                        // TODO Auto-generated catch block
                                        e.printStackTrace();
                ...
                for (QuestionWidget qw : odkView.getWidgets()) {
                    if (!qw.getFormEntryPrompt().isReadOnly()) {
                        ...
                        if (qw instanceof StringWidget) {
                            ...
                            if(objBranchSessio.getE_code().equals("")&& objBranchSessio.getE_code()!="code") {
                                String saveNameUpdateu = formController.getSubmissionMetadataUpdate(objBranchSessio.getE_code(),objBranchSessio.getE_rutaaggregate(),objBranchSessio.getE_nuevo());
                            }else{
                                if(objBranchSessio.getE_code()!="code"){
                                    String saveNameUpdateu = formController.getSubmissionMetadataUpdate(objBranchSessio.getE_code(), objBranchSessio.getE_rutaaggregate(), objBranchSessio.getE_nuevo());
                                }
                            }
                        } else {
